EESchema Schematic File Version 2
LIBS:power
LIBS:device
LIBS:transistors
LIBS:conn
LIBS:linear
LIBS:regul
LIBS:74xx
LIBS:cmos4000
LIBS:adc-dac
LIBS:memory
LIBS:xilinx
LIBS:microcontrollers
LIBS:dsp
LIBS:microchip
LIBS:analog_switches
LIBS:motorola
LIBS:texas
LIBS:intel
LIBS:audio
LIBS:interface
LIBS:digital-audio
LIBS:philips
LIBS:display
LIBS:cypress
LIBS:siliconi
LIBS:opto
LIBS:atmel
LIBS:contrib
LIBS:valves
LIBS:74540
LIBS:Befehle-cache
EELAYER 25 0
EELAYER END
$Descr A4 11693 8268
encoding utf-8
Sheet 1 1
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Comp
L R R7
U 1 1 57E93BBB
P -4550 200
F 0 "R7" V -4757 200 50  0000 C CNN
F 1 "330" V -4666 200 50  0000 C CNN
F 2 "Resistors_ThroughHole:R_Axial_DIN0204_L3.6mm_D1.6mm_P5.08mm_Horizontal" V -4620 200 50  0000 C CNN
F 3 "" H -4550 200 50  0000 C CNN
	1    -4550 200 
	-1   0    0    1   
$EndComp
$Comp
L R R8
U 1 1 57E93CDA
P -4450 200
F 0 "R8" V -4657 200 50  0000 C CNN
F 1 "330" V -4566 200 50  0000 C CNN
F 2 "Resistors_ThroughHole:R_Axial_DIN0204_L3.6mm_D1.6mm_P5.08mm_Horizontal" V -4520 200 50  0000 C CNN
F 3 "" H -4450 200 50  0000 C CNN
	1    -4450 200 
	-1   0    0    1   
$EndComp
$Comp
L R R10
U 1 1 57E93D22
P -4350 200
F 0 "R10" V -4557 200 50  0000 C CNN
F 1 "330" V -4466 200 50  0000 C CNN
F 2 "Resistors_ThroughHole:R_Axial_DIN0204_L3.6mm_D1.6mm_P5.08mm_Horizontal" V -4420 200 50  0000 C CNN
F 3 "" H -4350 200 50  0000 C CNN
	1    -4350 200 
	-1   0    0    1   
$EndComp
$Comp
L R R11
U 1 1 57E93D69
P -4250 200
F 0 "R11" V -4457 200 50  0000 C CNN
F 1 "330" V -4366 200 50  0000 C CNN
F 2 "Resistors_ThroughHole:R_Axial_DIN0204_L3.6mm_D1.6mm_P5.08mm_Horizontal" V -4320 200 50  0000 C CNN
F 3 "" H -4250 200 50  0000 C CNN
	1    -4250 200 
	-1   0    0    1   
$EndComp
$Comp
L R R12
U 1 1 57E93DAB
P -4150 200
F 0 "R12" V -4357 200 50  0000 C CNN
F 1 "330" V -4266 200 50  0000 C CNN
F 2 "Resistors_ThroughHole:R_Axial_DIN0204_L3.6mm_D1.6mm_P5.08mm_Horizontal" V -4220 200 50  0000 C CNN
F 3 "" H -4150 200 50  0000 C CNN
	1    -4150 200 
	-1   0    0    1   
$EndComp
$Comp
L R R13
U 1 1 57E93DE4
P -4050 200
F 0 "R13" V -4257 200 50  0000 C CNN
F 1 "330" V -4166 200 50  0000 C CNN
F 2 "Resistors_ThroughHole:R_Axial_DIN0204_L3.6mm_D1.6mm_P5.08mm_Horizontal" V -4120 200 50  0000 C CNN
F 3 "" H -4050 200 50  0000 C CNN
	1    -4050 200 
	-1   0    0    1   
$EndComp
$Comp
L R R15
U 1 1 57E93E32
P -3950 200
F 0 "R15" V -4157 200 50  0000 C CNN
F 1 "330" V -4066 200 50  0000 C CNN
F 2 "Resistors_ThroughHole:R_Axial_DIN0204_L3.6mm_D1.6mm_P5.08mm_Horizontal" V -4020 200 50  0000 C CNN
F 3 "" H -3950 200 50  0000 C CNN
	1    -3950 200 
	-1   0    0    1   
$EndComp
$Comp
L R R18
U 1 1 57E93E85
P -3850 200
F 0 "R18" V -4057 200 50  0000 C CNN
F 1 "330" V -3966 200 50  0000 C CNN
F 2 "Resistors_ThroughHole:R_Axial_DIN0204_L3.6mm_D1.6mm_P5.08mm_Horizontal" V -3920 200 50  0000 C CNN
F 3 "" H -3850 200 50  0000 C CNN
	1    -3850 200 
	-1   0    0    1   
$EndComp
$Comp
L Led_Small D5
U 1 1 57E94020
P -4550 -50
F 0 "D5" H -4550 -163 50  0000 C CNN
F 1 "Led_Small" H -4550 -164 50  0001 C CNN
F 2 "LEDs:LED_D3.0mm" V -4550 -50 50  0000 C CNN
F 3 "" V -4550 -50 50  0000 C CNN
	1    -4550 -50 
	0    -1   -1   0   
$EndComp
$Comp
L Led_Small D6
U 1 1 57E94088
P -4450 -50
F 0 "D6" H -4450 -163 50  0000 C CNN
F 1 "Led_Small" H -4450 -164 50  0001 C CNN
F 2 "LEDs:LED_D3.0mm" V -4450 -50 50  0000 C CNN
F 3 "" V -4450 -50 50  0000 C CNN
	1    -4450 -50 
	0    -1   -1   0   
$EndComp
$Comp
L Led_Small D8
U 1 1 57E940D8
P -4350 -50
F 0 "D8" H -4350 -163 50  0000 C CNN
F 1 "Led_Small" H -4350 -164 50  0001 C CNN
F 2 "LEDs:LED_D3.0mm" V -4350 -50 50  0000 C CNN
F 3 "" V -4350 -50 50  0000 C CNN
	1    -4350 -50 
	0    -1   -1   0   
$EndComp
$Comp
L Led_Small D9
U 1 1 57E9412E
P -4250 -50
F 0 "D9" H -4250 -163 50  0000 C CNN
F 1 "Led_Small" H -4250 -164 50  0001 C CNN
F 2 "LEDs:LED_D3.0mm" V -4250 -50 50  0000 C CNN
F 3 "" V -4250 -50 50  0000 C CNN
	1    -4250 -50 
	0    -1   -1   0   
$EndComp
$Comp
L Led_Small D10
U 1 1 57E94186
P -4150 -50
F 0 "D10" H -4150 -163 50  0000 C CNN
F 1 "Led_Small" H -4150 -164 50  0001 C CNN
F 2 "LEDs:LED_D3.0mm" V -4150 -50 50  0000 C CNN
F 3 "" V -4150 -50 50  0000 C CNN
	1    -4150 -50 
	0    -1   -1   0   
$EndComp
$Comp
L Led_Small D13
U 1 1 57E941E2
P -4050 -50
F 0 "D13" H -4050 -163 50  0000 C CNN
F 1 "Led_Small" H -4050 -164 50  0001 C CNN
F 2 "LEDs:LED_D3.0mm" V -4050 -50 50  0000 C CNN
F 3 "" V -4050 -50 50  0000 C CNN
	1    -4050 -50 
	0    -1   -1   0   
$EndComp
$Comp
L Led_Small D14
U 1 1 57E94242
P -3950 -50
F 0 "D14" H -3950 -163 50  0000 C CNN
F 1 "Led_Small" H -3950 -164 50  0001 C CNN
F 2 "LEDs:LED_D3.0mm" V -3950 -50 50  0000 C CNN
F 3 "" V -3950 -50 50  0000 C CNN
	1    -3950 -50 
	0    -1   -1   0   
$EndComp
$Comp
L Led_Small D18
U 1 1 57E942A6
P -3850 -50
F 0 "D18" H -3850 -163 50  0000 C CNN
F 1 "LED" H -3850 -164 50  0001 C CNN
F 2 "LEDs:LED_D3.0mm" V -3850 -50 50  0000 C CNN
F 3 "" V -3850 -50 50  0000 C CNN
	1    -3850 -50 
	0    -1   -1   0   
$EndComp
$Comp
L 74LS373 U6
U 1 1 57E94D2E
P -6400 4000
F 0 "U6" H -6400 3234 50  0000 C CNN
F 1 "74LS373" H -6400 3325 50  0000 C CNN
F 2 "Housings_DIP:DIP-20_W7.62mm_Socket_LongPads" H -6400 4000 50  0000 C CNN
F 3 "" H -6400 4000 50  0000 C CNN
	1    -6400 4000
	0    -1   -1   0   
$EndComp
$Comp
L 74LS541 U5
U 1 1 57E94F18
P -6400 1200
F 0 "U5" H -6400 434 50  0000 C CNN
F 1 "74LS541" H -6400 525 50  0000 C CNN
F 2 "Housings_DIP:DIP-20_W7.62mm_Socket_LongPads" H -6400 1200 50  0000 C CNN
F 3 "" H -6400 1200 50  0000 C CNN
	1    -6400 1200
	0    -1   -1   0   
$EndComp
$Comp
L 74LS139 U7
U 1 1 57E9588C
P -5000 6100
F 0 "U7" V -4954 5657 50  0000 R CNN
F 1 "74LS139" V -5045 5657 50  0000 R CNN
F 2 "Housings_DIP:DIP-16_W7.62mm_Socket_LongPads" H -5000 6100 50  0000 C CNN
F 3 "" H -5000 6100 50  0000 C CNN
	1    -5000 6100
	1    0    0    -1  
$EndComp
$Comp
L R R14
U 1 1 57E98FC1
P -4050 5250
F 0 "R14" V -4257 5250 50  0000 C CNN
F 1 "10k" V -4166 5250 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -4120 5250 50  0000 C CNN
F 3 "" H -4050 5250 50  0000 C CNN
	1    -4050 5250
	-1   0    0    1   
$EndComp
$Comp
L R R16
U 1 1 57E992B4
P -3900 5250
F 0 "R16" V -4107 5250 50  0000 C CNN
F 1 "10k" V -4016 5250 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -3970 5250 50  0000 C CNN
F 3 "" H -3900 5250 50  0000 C CNN
	1    -3900 5250
	-1   0    0    1   
$EndComp
$Comp
L R R19
U 1 1 57E99353
P -3750 5250
F 0 "R19" V -3957 5250 50  0000 C CNN
F 1 "10k" V -3866 5250 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -3820 5250 50  0000 C CNN
F 3 "" H -3750 5250 50  0000 C CNN
	1    -3750 5250
	-1   0    0    1   
$EndComp
$Comp
L R R20
U 1 1 57E993B7
P -3600 5250
F 0 "R20" V -3807 5250 50  0000 C CNN
F 1 "10k" V -3716 5250 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -3670 5250 50  0000 C CNN
F 3 "" H -3600 5250 50  0000 C CNN
	1    -3600 5250
	-1   0    0    1   
$EndComp
$Comp
L 74LS04 U2
U 6 1 57E99B62
P -3350 8450
F 0 "U2" H -3350 8765 50  0000 C CNN
F 1 "74LS04" H -3350 8674 50  0000 C CNN
F 2 "" H -3350 8450 50  0000 C CNN
F 3 "" H -3350 8450 50  0000 C CNN
	6    -3350 8450
	0    1    1    0   
$EndComp
$Comp
L 74LS04 U2
U 5 1 57E99C21
P -3900 8450
F 0 "U2" H -3900 8765 50  0000 C CNN
F 1 "74LS04" H -3900 8674 50  0000 C CNN
F 2 "" H -3900 8450 50  0000 C CNN
F 3 "" H -3900 8450 50  0000 C CNN
	5    -3900 8450
	0    1    1    0   
$EndComp
$Comp
L 74LS04 U2
U 4 1 57E99C93
P -4400 8450
F 0 "U2" H -4400 8765 50  0000 C CNN
F 1 "74LS04" H -4400 8674 50  0000 C CNN
F 2 "" H -4400 8450 50  0000 C CNN
F 3 "" H -4400 8450 50  0000 C CNN
	4    -4400 8450
	0    1    1    0   
$EndComp
$Comp
L 74LS04 U2
U 3 1 57E99D12
P -4850 8450
F 0 "U2" H -4850 8765 50  0000 C CNN
F 1 "74LS04" H -4850 8674 50  0000 C CNN
F 2 "" H -4850 8450 50  0000 C CNN
F 3 "" H -4850 8450 50  0000 C CNN
	3    -4850 8450
	0    1    1    0   
$EndComp
$Comp
L R R21
U 1 1 57E99EFD
P -3350 9050
F 0 "R21" V -3557 9050 50  0000 C CNN
F 1 "330" V -3466 9050 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -3420 9050 50  0000 C CNN
F 3 "" H -3350 9050 50  0000 C CNN
	1    -3350 9050
	-1   0    0    1   
$EndComp
$Comp
L R R17
U 1 1 57E9A030
P -3900 9050
F 0 "R17" V -4107 9050 50  0000 C CNN
F 1 "330" V -4016 9050 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -3970 9050 50  0000 C CNN
F 3 "" H -3900 9050 50  0000 C CNN
	1    -3900 9050
	-1   0    0    1   
$EndComp
$Comp
L R R9
U 1 1 57E9A0AC
P -4400 9050
F 0 "R9" V -4607 9050 50  0000 C CNN
F 1 "330" V -4516 9050 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -4470 9050 50  0000 C CNN
F 3 "" H -4400 9050 50  0000 C CNN
	1    -4400 9050
	-1   0    0    1   
$EndComp
$Comp
L R R6
U 1 1 57E9A129
P -4850 9050
F 0 "R6" V -5057 9050 50  0000 C CNN
F 1 "330" V -4966 9050 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -4920 9050 50  0000 C CNN
F 3 "" H -4850 9050 50  0000 C CNN
	1    -4850 9050
	-1   0    0    1   
$EndComp
$Comp
L Led_Small D21
U 1 1 57E9A417
P -3350 9300
F 0 "D21" H -3350 9443 50  0000 C CNN
F 1 "LED" H -3350 9186 50  0001 C CNN
F 2 "LEDs:LED-3MM" V -3350 9300 50  0000 C CNN
F 3 "" V -3350 9300 50  0000 C CNN
	1    -3350 9300
	0    1    1    0   
$EndComp
$Comp
L Led_Small D17
U 1 1 57E9A814
P -3900 9300
F 0 "D17" H -3900 9443 50  0000 C CNN
F 1 "LED" H -3900 9186 50  0001 C CNN
F 2 "LEDs:LED-3MM" V -3900 9300 50  0000 C CNN
F 3 "" V -3900 9300 50  0000 C CNN
	1    -3900 9300
	0    1    1    0   
$EndComp
$Comp
L Led_Small D7
U 1 1 57E9A89E
P -4400 9300
F 0 "D7" H -4400 9443 50  0000 C CNN
F 1 "LED" H -4400 9186 50  0001 C CNN
F 2 "LEDs:LED-3MM" V -4400 9300 50  0000 C CNN
F 3 "" V -4400 9300 50  0000 C CNN
	1    -4400 9300
	0    1    1    0   
$EndComp
$Comp
L Led_Small D4
U 1 1 57E9A927
P -4850 9300
F 0 "D4" H -4850 9443 50  0000 C CNN
F 1 "LED" H -4850 9186 50  0001 C CNN
F 2 "LEDs:LED-3MM" V -4850 9300 50  0000 C CNN
F 3 "" V -4850 9300 50  0000 C CNN
	1    -4850 9300
	0    1    1    0   
$EndComp
$Comp
L 74HCT02 U4
U 1 1 57F2C440
P -7700 4500
F 0 "U4" H -7700 4550 50  0000 C CNN
F 1 "74HCT02" H -7650 4450 50  0000 C CNN
F 2 "Housings_DIP:DIP-14_W7.62mm_Socket_LongPads" H -7700 4500 50  0000 C CNN
F 3 "" H -7700 4500 50  0000 C CNN
	1    -7700 4500
	-1   0    0    1   
$EndComp
$Comp
L 74HCT02 U4
U 2 1 57F2C605
P -7700 5050
F 0 "U4" H -7700 5100 50  0000 C CNN
F 1 "74HCT02" H -7650 5000 50  0000 C CNN
F 2 "Housings_DIP:DIP-14_W7.62mm_Socket_LongPads" H -7700 5050 50  0000 C CNN
F 3 "" H -7700 5050 50  0000 C CNN
	2    -7700 5050
	-1   0    0    1   
$EndComp
$Comp
L 74HCT02 U4
U 3 1 57F2C694
P -7700 5600
F 0 "U4" H -7700 5650 50  0000 C CNN
F 1 "74HCT02" H -7650 5550 50  0000 C CNN
F 2 "Housings_DIP:DIP-14_W7.62mm_Socket_LongPads" H -7700 5600 50  0000 C CNN
F 3 "" H -7700 5600 50  0000 C CNN
	3    -7700 5600
	-1   0    0    1   
$EndComp
$Comp
L 74HCT02 U4
U 4 1 57F2C76F
P -7700 6150
F 0 "U4" H -7700 6200 50  0000 C CNN
F 1 "74HCT02" H -7650 6100 50  0000 C CNN
F 2 "Housings_DIP:DIP-14_W7.62mm_Socket_LongPads" H -7700 6150 50  0000 C CNN
F 3 "" H -7700 6150 50  0000 C CNN
	4    -7700 6150
	-1   0    0    1   
$EndComp
$Comp
L 74LS540 U8
U 1 1 57E93AFB
P -4050 1050
F 0 "U8" H -4050 284 50  0000 C CNN
F 1 "74LS540" H -4050 375 50  0000 C CNN
F 2 "Housings_DIP:DIP-20_W7.62mm_Socket_LongPads" H -4050 1050 50  0000 C CNN
F 3 "" H -4050 1050 50  0000 C CNN
	1    -4050 1050
	0    -1   -1   0   
$EndComp
$Comp
L 7400 U3
U 2 1 57F3278C
P -8400 7250
F 0 "U3" H -8400 7300 50  0000 C CNN
F 1 "7400" H -8400 7150 50  0000 C CNN
F 2 "Housings_DIP:DIP-14_W7.62mm_Socket_LongPads" H -8400 7250 50  0000 C CNN
F 3 "" H -8400 7250 50  0000 C CNN
	2    -8400 7250
	0    1    1    0   
$EndComp
$Comp
L 7400 U3
U 3 1 57F32988
P -8300 8450
F 0 "U3" H -8300 8500 50  0000 C CNN
F 1 "7400" H -8300 8350 50  0000 C CNN
F 2 "Housings_DIP:DIP-14_W7.62mm_Socket_LongPads" H -8300 8450 50  0000 C CNN
F 3 "" H -8300 8450 50  0000 C CNN
	3    -8300 8450
	0    1    1    0   
$EndComp
$Comp
L 7400 U3
U 4 1 57F32A39
P -7650 8850
F 0 "U3" H -7650 8900 50  0000 C CNN
F 1 "7400" H -7650 8750 50  0000 C CNN
F 2 "Housings_DIP:DIP-14_W7.62mm_Socket_LongPads" H -7650 8850 50  0000 C CNN
F 3 "" H -7650 8850 50  0000 C CNN
	4    -7650 8850
	0    1    1    0   
$EndComp
$Comp
L 7400 U3
U 1 1 57F331B6
P -8650 6050
F 0 "U3" H -8650 6100 50  0000 C CNN
F 1 "7400" H -8650 5950 50  0000 C CNN
F 2 "Housings_DIP:DIP-14_W7.62mm_Socket_LongPads" H -8650 6050 50  0000 C CNN
F 3 "" H -8650 6050 50  0000 C CNN
	1    -8650 6050
	0    1    1    0   
$EndComp
$Comp
L R R4
U 1 1 57F34F54
P -8300 3850
F 0 "R4" V -8220 3850 50  0000 C CNN
F 1 "1k" V -8300 3850 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -8370 3850 50  0000 C CNN
F 3 "" H -8300 3850 50  0000 C CNN
	1    -8300 3850
	-1   0    0    1   
$EndComp
$Comp
L 74LS14 U1
U 1 1 57FBE09D
P -9250 950
F 0 "U1" H -9055 1065 50  0000 C CNN
F 1 "4069" H -9060 825 50  0000 C CNN
F 2 "" H -9250 950 60  0001 C CNN
F 3 "" H -9250 950 60  0001 C CNN
	1    -9250 950 
	1    0    0    -1  
$EndComp
$Comp
L 4069 U1
U 3 1 57FBE380
P -8300 950
F 0 "U1" H -8105 1065 50  0000 C CNN
F 1 "4069" H -8110 825 50  0000 C CNN
F 2 "" H -8300 950 60  0001 C CNN
F 3 "" H -8300 950 60  0001 C CNN
	3    -8300 950 
	1    0    0    -1  
$EndComp
$Comp
L 4069 U1
U 2 1 57FBEDD2
P -8800 1400
F 0 "U1" H -8605 1515 50  0000 C CNN
F 1 "4069" H -8610 1275 50  0000 C CNN
F 2 "" H -8800 1400 60  0001 C CNN
F 3 "" H -8800 1400 60  0001 C CNN
	2    -8800 1400
	0    1    1    0   
$EndComp
$Comp
L R R3
U 1 1 57FBEFF9
P -8800 2000
F 0 "R3" V -8720 2000 50  0000 C CNN
F 1 "330" V -8800 2000 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -8870 2000 50  0000 C CNN
F 3 "" H -8800 2000 50  0000 C CNN
	1    -8800 2000
	-1   0    0    1   
$EndComp
$Comp
L Led_Small D2
U 1 1 57FBF0ED
P -8800 2250
F 0 "D2" H -8850 2375 50  0000 L CNN
F 1 "Led_Small" H -8975 2150 50  0000 L CNN
F 2 "LEDs:LED-3MM" V -8800 2250 50  0000 C CNN
F 3 "" V -8800 2250 50  0000 C CNN
	1    -8800 2250
	0    1    1    0   
$EndComp
$Comp
L VCC #PWR5
U 1 1 57FBF211
P -8800 2450
F 0 "#PWR5" H -8800 2300 50  0001 C CNN
F 1 "VCC" H -8800 2600 50  0000 C CNN
F 2 "" H -8800 2450 50  0000 C CNN
F 3 "" H -8800 2450 50  0000 C CNN
	1    -8800 2450
	-1   0    0    1   
$EndComp
$Comp
L R R1
U 1 1 57FBF2F2
P -9700 1100
F 0 "R1" V -9620 1100 50  0000 C CNN
F 1 "4,7k" V -9700 1100 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -9770 1100 50  0000 C CNN
F 3 "" H -9700 1100 50  0000 C CNN
	1    -9700 1100
	-1   0    0    1   
$EndComp
$Comp
L VCC #PWR1
U 1 1 57FBF424
P -9700 1250
F 0 "#PWR1" H -9700 1100 50  0001 C CNN
F 1 "VCC" H -9700 1400 50  0000 C CNN
F 2 "" H -9700 1250 50  0000 C CNN
F 3 "" H -9700 1250 50  0000 C CNN
	1    -9700 1250
	-1   0    0    1   
$EndComp
Text GLabel -9800 950  0    60   Input ~ 0
enableR
$Comp
L GND #PWR10
U 1 1 57FC32E9
P -6200 1900
F 0 "#PWR10" H -6200 1650 50  0001 C CNN
F 1 "GND" H -6200 1750 50  0000 C CNN
F 2 "" H -6200 1900 50  0000 C CNN
F 3 "" H -6200 1900 50  0000 C CNN
	1    -6200 1900
	0    -1   -1   0   
$EndComp
$Comp
L GND #PWR9
U 1 1 57FC33C9
P -6300 1900
F 0 "#PWR9" H -6300 1650 50  0001 C CNN
F 1 "GND" H -6300 1750 50  0000 C CNN
F 2 "" H -6300 1900 50  0000 C CNN
F 3 "" H -6300 1900 50  0000 C CNN
	1    -6300 1900
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR13
U 1 1 57FC4CCE
P -3600 1750
F 0 "#PWR13" H -3600 1500 50  0001 C CNN
F 1 "GND" H -3600 1600 50  0000 C CNN
F 2 "" H -3600 1750 50  0000 C CNN
F 3 "" H -3600 1750 50  0000 C CNN
	1    -3600 1750
	1    0    0    -1  
$EndComp
$Comp
L VCC #PWR14
U 1 1 57FC686E
P -3450 5100
F 0 "#PWR14" H -3450 4950 50  0001 C CNN
F 1 "VCC" H -3450 5250 50  0000 C CNN
F 2 "" H -3450 5100 50  0000 C CNN
F 3 "" H -3450 5100 50  0000 C CNN
	1    -3450 5100
	0    1    1    0   
$EndComp
$Comp
L VCC #PWR15
U 1 1 57FC839A
P -3200 9450
F 0 "#PWR15" H -3200 9300 50  0001 C CNN
F 1 "VCC" H -3200 9600 50  0000 C CNN
F 2 "" H -3200 9450 50  0000 C CNN
F 3 "" H -3200 9450 50  0000 C CNN
	1    -3200 9450
	-1   0    0    1   
$EndComp
Text GLabel -4600 6800 0    60   Input ~ 0
0
Text GLabel -4550 7050 0    60   Input ~ 0
1
Text GLabel -4450 7250 0    60   Input ~ 0
2
Text GLabel -4300 7450 0    60   Input ~ 0
3
$Comp
L VCC #PWR7
U 1 1 57FCF846
P -8300 3700
F 0 "#PWR7" H -8300 3550 50  0001 C CNN
F 1 "VCC" H -8300 3850 50  0000 C CNN
F 2 "" H -8300 3700 50  0000 C CNN
F 3 "" H -8300 3700 50  0000 C CNN
	1    -8300 3700
	1    0    0    -1  
$EndComp
$Comp
L Led_Small D15
U 1 1 580EAA2D
P -3950 5550
F 0 "D15" H -4000 5675 50  0000 L CNN
F 1 "Led_Small" H -4125 5450 50  0000 L CNN
F 2 "LEDs:LED-3MM" V -3950 5550 50  0000 C CNN
F 3 "" V -3950 5550 50  0000 C CNN
	1    -3950 5550
	0    1    1    0   
$EndComp
$Comp
L Led_Small D11
U 1 1 580EAB66
P -4100 5550
F 0 "D11" H -4150 5675 50  0000 L CNN
F 1 "Led_Small" H -4275 5450 50  0000 L CNN
F 2 "LEDs:LED-3MM" V -4100 5550 50  0000 C CNN
F 3 "" V -4100 5550 50  0000 C CNN
	1    -4100 5550
	0    1    1    0   
$EndComp
$Comp
L Led_Small D12
U 1 1 580EABEB
P -4100 5900
F 0 "D12" H -4150 6025 50  0000 L CNN
F 1 "Led_Small" H -4275 5800 50  0000 L CNN
F 2 "LEDs:LED-3MM" V -4100 5900 50  0000 C CNN
F 3 "" V -4100 5900 50  0000 C CNN
	1    -4100 5900
	0    1    1    0   
$EndComp
$Comp
L Led_Small D16
U 1 1 580EAC8E
P -3950 6150
F 0 "D16" H -4000 6275 50  0000 L CNN
F 1 "Led_Small" H -4125 6050 50  0000 L CNN
F 2 "LEDs:LED-3MM" V -3950 6150 50  0000 C CNN
F 3 "" V -3950 6150 50  0000 C CNN
	1    -3950 6150
	0    1    1    0   
$EndComp
$Comp
L Led_Small D20
U 1 1 580EAD43
P -3650 6400
F 0 "D20" H -3700 6525 50  0000 L CNN
F 1 "Led_Small" H -3825 6300 50  0000 L CNN
F 2 "LEDs:LED-3MM" V -3650 6400 50  0000 C CNN
F 3 "" V -3650 6400 50  0000 C CNN
	1    -3650 6400
	0    1    1    0   
$EndComp
$Comp
L Led_Small D19
U 1 1 580EAE00
P -3850 6400
F 0 "D19" H -3900 6525 50  0000 L CNN
F 1 "Led_Small" H -4025 6300 50  0000 L CNN
F 2 "LEDs:LED-3MM" V -3850 6400 50  0000 C CNN
F 3 "" V -3850 6400 50  0000 C CNN
	1    -3850 6400
	0    1    1    0   
$EndComp
$Comp
L GND #PWR12
U 1 1 580EBD54
P -5850 6350
F 0 "#PWR12" H -5850 6100 50  0001 C CNN
F 1 "GND" H -5850 6200 50  0000 C CNN
F 2 "" H -5850 6350 50  0000 C CNN
F 3 "" H -5850 6350 50  0000 C CNN
	1    -5850 6350
	0    1    1    0   
$EndComp
$Comp
L VCC #PWR11
U 1 1 580EC580
P -5900 4700
F 0 "#PWR11" H -5900 4550 50  0001 C CNN
F 1 "VCC" H -5900 4850 50  0000 C CNN
F 2 "" H -5900 4700 50  0000 C CNN
F 3 "" H -5900 4700 50  0000 C CNN
	1    -5900 4700
	0    1    1    0   
$EndComp
Text GLabel -8850 9350 0    60   Input ~ 0
upPC
Text GLabel -8900 7850 0    60   Input ~ 0
incPC
$Comp
L R 4,7k3
U 1 1 580F54F3
P -8700 8250
F 0 "4,7k3" V -8620 8250 50  0000 C CNN
F 1 "R" V -8700 8250 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -8770 8250 50  0000 C CNN
F 3 "" H -8700 8250 50  0000 C CNN
	1    -8700 8250
	-1   0    0    -1  
$EndComp
$Comp
L VCC #PWR6
U 1 1 580F57E4
P -8700 8400
F 0 "#PWR6" H -8700 8250 50  0001 C CNN
F 1 "VCC" H -8700 8550 50  0000 C CNN
F 2 "" H -8700 8400 50  0000 C CNN
F 3 "" H -8700 8400 50  0000 C CNN
	1    -8700 8400
	-1   0    0    1   
$EndComp
Text GLabel -9350 5350 0    60   Input ~ 0
incPCz
$Comp
L R 4,7k1
U 1 1 582A32C5
P -9150 5750
F 0 "4,7k1" V -9070 5750 50  0000 C CNN
F 1 "R" V -9150 5750 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -9220 5750 50  0000 C CNN
F 3 "" H -9150 5750 50  0000 C CNN
	1    -9150 5750
	-1   0    0    -1  
$EndComp
$Comp
L VCC #PWR3
U 1 1 582A32CB
P -9150 5900
F 0 "#PWR3" H -9150 5750 50  0001 C CNN
F 1 "VCC" H -9150 6050 50  0000 C CNN
F 2 "" H -9150 5900 50  0000 C CNN
F 3 "" H -9150 5900 50  0000 C CNN
	1    -9150 5900
	-1   0    0    1   
$EndComp
$Comp
L 74LS04 U2
U 1 1 582A9693
P -8900 4500
F 0 "U2" H -8900 4815 50  0000 C CNN
F 1 "74LS04" H -8900 4724 50  0000 C CNN
F 2 "" H -8900 4500 50  0000 C CNN
F 3 "" H -8900 4500 50  0000 C CNN
	1    -8900 4500
	-1   0    0    1   
$EndComp
$Comp
L R R2
U 1 1 582AA727
P -9350 4650
F 0 "R2" V -9557 4650 50  0000 C CNN
F 1 "330" V -9466 4650 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -9420 4650 50  0000 C CNN
F 3 "" H -9350 4650 50  0000 C CNN
	1    -9350 4650
	-1   0    0    1   
$EndComp
$Comp
L Led_Small D1
U 1 1 582AC971
P -9350 4900
F 0 "D1" H -9350 5043 50  0000 C CNN
F 1 "LED" H -9350 4786 50  0001 C CNN
F 2 "LEDs:LED-3MM" V -9350 4900 50  0000 C CNN
F 3 "" V -9350 4900 50  0000 C CNN
	1    -9350 4900
	0    1    1    0   
$EndComp
$Comp
L VCC #PWR2
U 1 1 582B09F9
P -9500 5000
F 0 "#PWR2" H -9500 4850 50  0001 C CNN
F 1 "VCC" H -9500 5150 50  0000 C CNN
F 2 "" H -9500 5000 50  0000 C CNN
F 3 "" H -9500 5000 50  0000 C CNN
	1    -9500 5000
	1    0    0    -1  
$EndComp
$Comp
L 74LS04 U2
U 2 1 582B5165
P -7750 3000
F 0 "U2" H -7750 3315 50  0000 C CNN
F 1 "74LS04" H -7750 3224 50  0000 C CNN
F 2 "" H -7750 3000 50  0000 C CNN
F 3 "" H -7750 3000 50  0000 C CNN
	2    -7750 3000
	1    0    0    -1  
$EndComp
Text GLabel -9100 3150 0    60   Input ~ 0
loadIR
$Comp
L R 4,7k2
U 1 1 582B55B4
P -8850 3450
F 0 "4,7k2" V -8770 3450 50  0000 C CNN
F 1 "R" V -8850 3450 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -8920 3450 50  0000 C CNN
F 3 "" H -8850 3450 50  0000 C CNN
	1    -8850 3450
	-1   0    0    -1  
$EndComp
$Comp
L VCC #PWR4
U 1 1 582B5718
P -8850 3600
F 0 "#PWR4" H -8850 3450 50  0001 C CNN
F 1 "VCC" H -8850 3750 50  0000 C CNN
F 2 "" H -8850 3600 50  0000 C CNN
F 3 "" H -8850 3600 50  0000 C CNN
	1    -8850 3600
	-1   0    0    1   
$EndComp
$Comp
L R R5
U 1 1 582B6652
P -7400 2350
F 0 "R5" V -7320 2350 50  0000 C CNN
F 1 "330" V -7400 2350 50  0000 C CNN
F 2 "Resistors_ThroughHole:Resistor_Horizontal_RM10mm" V -7470 2350 50  0000 C CNN
F 3 "" H -7400 2350 50  0000 C CNN
	1    -7400 2350
	0    -1   -1   0   
$EndComp
$Comp
L Led_Small D3
U 1 1 582B68A1
P -7700 2350
F 0 "D3" H -7700 2493 50  0000 C CNN
F 1 "LED" H -7700 2236 50  0001 C CNN
F 2 "LEDs:LED-3MM" V -7700 2350 50  0000 C CNN
F 3 "" V -7700 2350 50  0000 C CNN
	1    -7700 2350
	1    0    0    -1  
$EndComp
$Comp
L GND #PWR8
U 1 1 582B6B6B
P -7750 2350
F 0 "#PWR8" H -7750 2100 50  0001 C CNN
F 1 "GND" H -7750 2200 50  0000 C CNN
F 2 "" H -7750 2350 50  0000 C CNN
F 3 "" H -7750 2350 50  0000 C CNN
	1    -7750 2350
	0    1    1    0   
$EndComp
Text Label -3850 1750 3    60   ~ 0
a1
Text Label -3950 1750 3    60   ~ 0
a2
Text Label -4050 1750 3    60   ~ 0
a3
Text Label -4150 1750 3    60   ~ 0
a4
Text Label -4250 1750 3    60   ~ 0
a5
Text Label -4350 1750 3    60   ~ 0
a6
Text Label -4450 1750 3    60   ~ 0
a7
Text Label -4550 1750 3    60   ~ 0
a8
Entry Wire Line
	-3850 1850 -3950 1950
Entry Wire Line
	-3950 1850 -4050 1950
Entry Wire Line
	-4050 1850 -4150 1950
Entry Wire Line
	-4150 1850 -4250 1950
Entry Wire Line
	-4250 1850 -4350 1950
Entry Wire Line
	-4350 1850 -4450 1950
Entry Wire Line
	-4450 1850 -4550 1950
Entry Wire Line
	-4550 1850 -4650 1950
Entry Wire Line
	-6300 2650 -6400 2750
Entry Wire Line
	-6400 2650 -6500 2750
Entry Wire Line
	-6500 2650 -6600 2750
Entry Wire Line
	-6600 2650 -6700 2750
Entry Wire Line
	-6700 2650 -6800 2750
Entry Wire Line
	-6800 2650 -6900 2750
Entry Wire Line
	-6300 2650 -6400 2750
Entry Wire Line
	-6400 2550 -6300 2650
Entry Wire Line
	-6500 2550 -6400 2650
Entry Wire Line
	-6600 2550 -6500 2650
Entry Wire Line
	-6700 2550 -6600 2650
Entry Wire Line
	-6800 2550 -6700 2650
Entry Wire Line
	-6900 2550 -6800 2650
Text Label -6400 1900 3    60   ~ 0
a3
Text Label -6500 1900 3    60   ~ 0
a4
Text Label -6600 1900 3    60   ~ 0
a5
Text Label -6700 1900 3    60   ~ 0
a6
Text Label -6800 1900 3    60   ~ 0
a7
Text Label -6900 1900 3    60   ~ 0
a8
Text Label -6400 3200 3    60   ~ 0
a3
Text Label -6500 3200 3    60   ~ 0
a4
Text Label -6600 3200 3    60   ~ 0
a5
Text Label -6700 3200 3    60   ~ 0
a6
Text Label -6800 3200 3    60   ~ 0
a7
Text Label -6900 3200 3    60   ~ 0
a8
Entry Wire Line
	-6200 4750 -6300 4850
Entry Wire Line
	-6300 4750 -6400 4850
Entry Wire Line
	-6400 4750 -6500 4850
Entry Wire Line
	-6500 4750 -6600 4850
Entry Wire Line
	-6600 4750 -6700 4850
Entry Wire Line
	-6700 4750 -6800 4850
Entry Wire Line
	-6800 4750 -6900 4850
Entry Wire Line
	-6900 4750 -7000 4850
Entry Wire Line
	-6300 4900 -6400 5000
Entry Wire Line
	-6300 5050 -6400 5150
Entry Wire Line
	-6300 5400 -6400 5500
Entry Wire Line
	-6300 5600 -6400 5700
Entry Wire Line
	-6300 5950 -6400 6050
Entry Wire Line
	-6300 6150 -6400 6250
Text Label -6200 4600 3    60   ~ 0
d1
Text Label -6300 4600 3    60   ~ 0
d2
Text Label -6400 4600 3    60   ~ 0
d3
Text Label -6500 4600 3    60   ~ 0
d4
Text Label -6600 4600 3    60   ~ 0
d5
Text Label -6700 4600 3    60   ~ 0
d6
Text Label -6800 4600 3    60   ~ 0
d7
Text Label -7200 4400 1    60   ~ 0
d1
Text Label -7200 4600 1    60   ~ 0
d2
Text Label -7200 4950 1    60   ~ 0
d3
Text Label -7200 5150 1    60   ~ 0
d4
Text Label -7200 5500 1    60   ~ 0
d5
Text Label -7200 5700 1    60   ~ 0
d6
Text Label -7200 6050 1    60   ~ 0
d7
Text Label -7200 6250 1    60   ~ 0
d8
Entry Wire Line
	-5900 6800 -6000 6900
Entry Wire Line
	-6000 6800 -6100 6900
Entry Wire Line
	-6100 6800 -6200 6900
Entry Wire Line
	-6200 6800 -6300 6900
Entry Wire Line
	-6300 6800 -6400 6900
Entry Wire Line
	-6400 6800 -6500 6900
Entry Wire Line
	-6500 6800 -6600 6900
Text Label -6000 6950 1    60   ~ 0
d1
Text Label -6100 6950 1    60   ~ 0
d2
Text Label -6200 6950 1    60   ~ 0
d3
Text Label -6300 6950 1    60   ~ 0
d4
Text Label -6400 6950 1    60   ~ 0
d5
Text Label -6500 6950 1    60   ~ 0
d6
Text Label -6600 6950 1    60   ~ 0
d7
Text Label -8700 -850 1    60   ~ 0
y1
Text Label -8700 -750 1    60   ~ 0
y2
Text Label -8700 -650 1    60   ~ 0
y3
Text Label -8700 -550 1    60   ~ 0
y4
Text Label -8700 -450 1    60   ~ 0
y5
Text Label -8700 -350 1    60   ~ 0
y6
Text Label -8700 -250 1    60   ~ 0
y7
Text Label -6200 550  1    60   ~ 0
y1
Text Label -6300 550  1    60   ~ 0
y2
Text Label -6400 550  1    60   ~ 0
y3
Text Label -6500 550  1    60   ~ 0
y4
Text Label -6600 550  1    60   ~ 0
y5
Text Label -6700 550  1    60   ~ 0
y6
Text Label -6800 550  1    60   ~ 0
y7
Text Label -6900 550  1    60   ~ 0
y8
$Comp
L CONN_02X10 P2
U 1 1 58D096FC
P -6000 -800
F 0 "P2" V -5954 -1338 50  0000 R CNN
F 1 "CONN_02X10" V -6045 -1338 50  0000 R CNN
F 2 "Connect:IDC_Header_Straight_20pins" H -6000 -2000 50  0000 C CNN
F 3 "" H -6000 -2000 50  0000 C CNN
	1    -6000 -800
	0    -1   -1   0   
$EndComp
$Comp
L CONN_02X10 P1
U 1 1 5A1624C1
P -6350 7300
F 0 "P1" V -6304 6762 50  0000 R CNN
F 1 "CONN_02X10" V -6395 6762 50  0000 R CNN
F 2 "Connect:IDC_Header_Straight_20pins" H -6350 6100 50  0000 C CNN
F 3 "" H -6350 6100 50  0000 C CNN
	1    -6350 7300
	0    -1   -1   0   
$EndComp
Text Label -6950 6250 0    60   ~ 0
11
Text Label -6900 4750 0    60   ~ 0
d8
Wire Wire Line
	-8100 9350 -8850 9350
Wire Wire Line
	-9800 950  -9700 950 
Connection ~ -7650 8250
Wire Wire Line
	-7750 8250 -7550 8250
Wire Wire Line
	-7650 8150 -7650 8250
Wire Wire Line
	-7950 8150 -7650 8150
Wire Wire Line
	-7950 9050 -7950 8150
Wire Wire Line
	-8300 9050 -7950 9050
Wire Wire Line
	-6900 -400 -6900 500 
Wire Wire Line
	-5650 -400 -6900 -400
Wire Wire Line
	-6800 -350 -6800 500 
Wire Wire Line
	-5750 -350 -6800 -350
Wire Wire Line
	-6700 -300 -6700 500 
Wire Wire Line
	-5850 -300 -6700 -300
Wire Wire Line
	-6600 -250 -6600 500 
Wire Wire Line
	-5950 -250 -6600 -250
Wire Wire Line
	-6500 -200 -6500 500 
Wire Wire Line
	-6050 -200 -6500 -200
Wire Wire Line
	-6400 -150 -6400 500 
Wire Wire Line
	-6150 -150 -6400 -150
Wire Wire Line
	-6300 350  -6300 500 
Wire Wire Line
	-6250 350  -6300 350 
Wire Wire Line
	-6200 -100 -6200 500 
Wire Wire Line
	-6350 -100 -6200 -100
Wire Wire Line
	-9050 -450 -9200 -450
Wire Wire Line
	-9050 -550 -9200 -550
Wire Wire Line
	-9050 -650 -9200 -650
Wire Wire Line
	-9050 -750 -9200 -750
Wire Wire Line
	-9050 -850 -9200 -850
Wire Wire Line
	-9050 -950 -9200 -950
Wire Wire Line
	-9050 -1050 -9200 -1050
Wire Wire Line
	-9200 -1150 -9050 -1150
Wire Bus Line
	-6500 6800 -5900 6800
Connection ~ -7050 4400
Wire Wire Line
	-7100 4400 -7050 4400
Connection ~ -7050 4600
Wire Wire Line
	-7050 4600 -7100 4600
Wire Wire Line
	-7100 5000 -6400 5000
Wire Wire Line
	-7100 5000 -7100 4950
Wire Wire Line
	-6400 5150 -7100 5150
Wire Wire Line
	-6400 5500 -7100 5500
Wire Wire Line
	-6400 5700 -7100 5700
Wire Wire Line
	-6400 6050 -7100 6050
Wire Wire Line
	-6400 6250 -7100 6250
Wire Bus Line
	-6300 4850 -6300 4950
Wire Bus Line
	-6300 4900 -6300 6800
Wire Bus Line
	-7000 4850 -6300 4850
Wire Wire Line
	-7300 3000 -7050 3000
Connection ~ -7050 3000
Wire Wire Line
	-7050 3000 -7050 4900
Wire Wire Line
	-7050 4900 -6000 4900
Wire Wire Line
	-6900 1900 -6900 2550
Wire Wire Line
	-6800 2550 -6800 1900
Wire Wire Line
	-6700 2550 -6700 1900
Wire Wire Line
	-6600 2550 -6600 1900
Wire Wire Line
	-6500 2550 -6500 1900
Wire Wire Line
	-6400 2550 -6400 1900
Wire Wire Line
	-6400 3300 -6400 2750
Wire Wire Line
	-6500 2750 -6500 3300
Wire Wire Line
	-6600 2750 -6600 3300
Wire Wire Line
	-6700 2750 -6700 3300
Wire Wire Line
	-6800 2750 -6800 3300
Wire Wire Line
	-6900 2750 -6900 3300
Wire Bus Line
	-6900 2650 -3950 2650
Wire Bus Line
	-3950 1950 -3950 2650
Wire Bus Line
	-4650 1950 -3950 1950
Wire Wire Line
	-3300 -150 -4550 -150
Wire Wire Line
	-5850 6000 -6100 6000
Wire Wire Line
	-6100 6000 -6100 5250
Wire Wire Line
	-6100 5250 -5350 5250
Wire Wire Line
	-5350 5250 -5350 3100
Wire Wire Line
	-5350 3100 -6200 3100
Wire Wire Line
	-6200 3100 -6200 3300
Wire Wire Line
	-5850 5850 -5950 5850
Wire Wire Line
	-5950 5850 -5950 5400
Wire Wire Line
	-5950 5400 -5250 5400
Wire Wire Line
	-5250 5400 -5250 3000
Wire Wire Line
	-5250 3000 -6300 3000
Wire Wire Line
	-6300 3000 -6300 3300
Wire Wire Line
	-3900 5400 -3900 7800
Wire Wire Line
	-4050 5400 -4050 7700
Wire Wire Line
	-3750 5400 -3750 8000
Wire Wire Line
	-3600 5400 -3600 7700
Wire Wire Line
	-3600 7700 -3350 7700
Wire Wire Line
	-3350 7700 -3350 8000
Wire Wire Line
	-4050 7700 -4850 7700
Wire Wire Line
	-4850 7700 -4850 8000
Wire Wire Line
	-3750 8000 -3900 8000
Wire Wire Line
	-4400 7800 -4400 8000
Wire Wire Line
	-3900 7800 -4400 7800
Wire Wire Line
	-4850 9450 -4850 9400
Wire Wire Line
	-4850 9450 -3200 9450
Wire Wire Line
	-3350 9450 -3350 9400
Connection ~ -3350 9450
Wire Wire Line
	-3900 9450 -3900 9400
Connection ~ -3900 9450
Wire Wire Line
	-4400 9450 -4400 9400
Connection ~ -4400 9450
Wire Wire Line
	-8300 4000 -8300 6650
Wire Wire Line
	-8500 6650 -8650 6650
Wire Wire Line
	-8900 7850 -8200 7850
Connection ~ -8300 4500
Wire Wire Line
	-8300 4500 -8450 4500
Wire Wire Line
	-8750 950  -8800 950 
Wire Wire Line
	-8800 2350 -8800 2450
Wire Wire Line
	-5900 2350 -5900 1900
Wire Wire Line
	-7050 2350 -5900 2350
Wire Wire Line
	-7050 2350 -7050 950 
Wire Wire Line
	-7050 950  -7850 950 
Wire Wire Line
	-6000 1900 -6000 2350
Connection ~ -6000 2350
Wire Wire Line
	-3550 1750 -3650 1750
Wire Wire Line
	-4050 5100 -3450 5100
Wire Wire Line
	-4050 7450 -4300 7450
Connection ~ -4050 7450
Wire Wire Line
	-3900 7250 -4450 7250
Connection ~ -3900 7250
Wire Wire Line
	-3750 7050 -4550 7050
Connection ~ -3750 7050
Wire Wire Line
	-3600 6800 -4600 6800
Connection ~ -3600 6800
Wire Wire Line
	-4150 5800 -4150 5650
Wire Wire Line
	-4150 5650 -3300 5650
Wire Wire Line
	-4150 6000 -3300 6000
Wire Wire Line
	-4150 6200 -4100 6200
Wire Wire Line
	-4100 6200 -4100 6250
Wire Wire Line
	-4100 6250 -3300 6250
Wire Wire Line
	-4150 6400 -4150 6500
Wire Wire Line
	-4150 6500 -3300 6500
Wire Wire Line
	-3650 6300 -3600 6300
Connection ~ -3600 6300
Wire Wire Line
	-3850 6300 -3750 6300
Connection ~ -3750 6300
Wire Wire Line
	-3900 6050 -3950 6050
Connection ~ -3900 6050
Wire Wire Line
	-4050 5800 -4100 5800
Connection ~ -4050 5800
Wire Wire Line
	-4100 5450 -4050 5450
Connection ~ -4050 5450
Wire Wire Line
	-3950 5450 -3900 5450
Connection ~ -3900 5450
Wire Wire Line
	-7650 9450 -8100 9450
Wire Wire Line
	-8100 9450 -8100 9350
Wire Wire Line
	-8700 7850 -8700 8100
Connection ~ -8700 7850
Wire Wire Line
	-9350 5350 -8650 5350
Wire Wire Line
	-9150 5350 -9150 5600
Connection ~ -9150 5350
Wire Wire Line
	-8750 5450 -8550 5450
Wire Wire Line
	-8650 5350 -8650 5450
Connection ~ -8650 5450
Wire Wire Line
	-9350 5000 -9500 5000
Wire Wire Line
	-6000 4900 -6000 4700
Wire Wire Line
	-8200 3150 -8200 3000
Wire Wire Line
	-9100 3150 -8200 3150
Wire Wire Line
	-8850 3150 -8850 3300
Connection ~ -8850 3150
Wire Wire Line
	-7250 3000 -7250 2350
Wire Wire Line
	-7500 2350 -7600 2350
Connection ~ -7250 3000
Wire Wire Line
	-3850 1750 -3850 1850
Wire Wire Line
	-3950 1750 -3950 1850
Wire Wire Line
	-4050 1750 -4050 1850
Wire Wire Line
	-4150 1750 -4150 1850
Wire Wire Line
	-4250 1750 -4250 1850
Wire Wire Line
	-4350 1750 -4350 1850
Wire Wire Line
	-4450 1750 -4450 1850
Wire Wire Line
	-4550 1750 -4550 1850
Wire Wire Line
	-5650 -550 -5650 -400
Wire Wire Line
	-5750 -550 -5750 -350
Wire Wire Line
	-5850 -550 -5850 -300
Wire Wire Line
	-5950 -550 -5950 -250
Wire Wire Line
	-6050 -550 -6050 -200
Wire Wire Line
	-6150 -550 -6150 -150
Wire Wire Line
	-6250 -550 -6250 350 
Wire Wire Line
	-6350 -550 -6350 -100
Wire Wire Line
	-6900 4700 -6900 4750
Wire Wire Line
	-6800 4700 -6800 4750
Wire Wire Line
	-6700 4700 -6700 4750
Wire Wire Line
	-6600 4700 -6600 4750
Wire Wire Line
	-6500 4700 -6500 4750
Wire Wire Line
	-6400 4700 -6400 4750
Wire Wire Line
	-6300 4700 -6300 4750
Wire Wire Line
	-6200 4700 -6200 4750
Wire Wire Line
	-42900 40500 -42900 25500
Wire Wire Line
	-58500 40500 -42900 40500
Wire Wire Line
	-58500 40500 -58500 25400
Wire Wire Line
	-58500 25400 -42900 25400
Wire Wire Line
	-42900 25400 -42900 10550
Wire Wire Line
	-42900 10550 -58250 10550
Wire Wire Line
	-58250 10550 -58250 20400
Wire Wire Line
	-58250 20400 -43000 20400
Wire Wire Line
	-43000 20400 -43000 19900
Wire Wire Line
	-55050 12950 -55050 15000
Wire Wire Line
	-55050 15000 -53000 15000
Wire Wire Line
	-53000 15000 -53000 14500
Wire Wire Line
	-48750 13500 -48750 15800
Wire Wire Line
	-48750 15800 -47000 15800
Wire Wire Line
	-53400 18850 -47600 18850
Wire Wire Line
	-47600 18850 -47600 17700
Wire Wire Line
	-58350 31550 -63100 31550
Wire Wire Line
	-63100 31550 -63100 39100
Wire Wire Line
	-42250 31650 -36350 31650
Wire Wire Line
	-36350 31650 -36350 41900
Wire Wire Line
	-47750 40600 -47750 49200
Wire Wire Line
	-47750 49200 -49550 49200
Wire Wire Line
	-53650 40500 -53650 48450
Wire Wire Line
	-53650 48450 -56350 48450
Connection ~ -53650 40500
$Comp
L 74LS540 U9
U 1 1 5A1F49EC
P -2250 7100
F 0 "U9" H -2250 6900 50  0000 C CNN
F 1 "74LS540" H -2250 6800 50  0000 C CNN
F 2 "" H -2250 7100 50  0000 C CNN
F 3 "" H -2250 7100 50  0000 C CNN
	1    -2250 7100
	1    0    0    -1  
$EndComp
$Comp
L 74540 U?
U 1 1 5A1FC465
P -2200 3350
F 0 "U?" H -2100 3450 60  0000 C CNN
F 1 "74540" H -2050 3200 60  0000 C CNN
F 2 "" H -2050 3200 60  0001 C CNN
F 3 "" H -2050 3200 60  0001 C CNN
	1    -2200 3350
	1    0    0    -1  
$EndComp
$EndSCHEMATC
